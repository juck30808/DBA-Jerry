%%% arxordersel.m; M file for ARX model order selection %%%

function [] = arxordersel( )
...
% Estimate ARX models between orders 1 and 8
% and caculate output error loss functions

for k = 1:8
  THarx001 = arx([Y001(1:N1) U(1:N1)],[k k 1]);
  Error001 = Y001 - idsim(U,THarx001);
  V001_est(k) = cov(Error001(1:N1));
  V001_eval(k) = cov(Error001(N1+1:N));
  THarx01 = arx([Y01(1:N1) U(1:N1)],[k k 1]);
  Error01 = Y01 - idsim(U,THarx01);
  V01_est(k) = cov(Error01(1:N1));
  V01_eval(k) = cov(Error01(N1+1:N));
  THarx10 = arx([Y10(1:N1) U(1:N1)],[k k 1]);
  Error10 = Y10 - idsim(U,THarx10);
  V10_est(k) = cov(Error10(1:N1));
  V10_eval(k) = cov(Error10(N1+1:N));
  THarx50 = arx([Y50(1:N1) U(1:N1)],[k k 1]);
  Error50 = Y50 - idsim(U,THarx50);
